...
temp_sheet = template.create_sheet('Sheet3',0)
# do stuff
...
for file in folder:
    ...
    try:
        ...
        for sh in sh_names:
            ...
            epid = ws0.cell(row=8 + int(sh)-1, column=6).value
            try:
                if epid[0:2]=='MA':
                    epid='IQE J+'
                elif epid[0:2]=='NA':
            ...
            for row in range(0, 2,1):
                param=ws.cell(row=13+row, column=1).value
                data=ws.cell(row=13+row, column=5).value
                temp_sheet.cell(row=1, column=m+2).value=param
                temp_sheet.cell(row=sheet_no, column=m+2).value = data
                m=m+1
            try:
